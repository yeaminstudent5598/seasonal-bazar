...
import { Search, MapPin, ShoppingBag, User, ChevronRight, Heart, Menu, Home as HomeIcon, LayoutGrid, X } from "lucide-react";
...
export default function Home() {
  ...
  return (
    <div className="min-h-screen flex flex-col font-sans bg-[#F0F2F5] pb-20 md:pb-0">
      ...
      {/* --- MOBILE DRAWER MENU --- */}
      <div 
        className={`fixed inset-0 z-[60] bg-black/50 transition-opacity duration-300 md:hidden ${isMobileMenuOpen ? "opacity-100 visible" : "opacity-0 invisible"}`}
        onClick={() => setIsMobileMenuOpen(false)}
      >
        <div 
          className={`fixed top-0 left-0 h-full w-[280px] bg-white shadow-2xl transition-transform duration-300 transform ${isMobileMenuOpen ? "translate-x-0" : "-translate-x-full"}`}
          onClick={(e) => e.stopPropagation()}
        >
            <div className="bg-[#800000] p-4 flex justify-between items-center text-white shadow-md">
                <div className="flex items-center gap-2">
                   <div className="w-8 h-8 bg-white rounded-full p-1 relative">
                      <Image src="/SeasonalBazar_Logo.png" alt="Logo" fill className="object-contain" />
                   </div>
                   <span className="font-bold text-lg">Categories</span>
                </div>
                <button onClick={() => setIsMobileMenuOpen(false)} className="p-1 hover:bg-white/20 rounded-full transition">
                   <X className="w-6 h-6" />
                </button>
            </div>
            <div className="overflow-y-auto h-[calc(100%-70px)] py-2">
                <ul className="space-y-0">
                    {categories.map((cat, idx) => (
                    <li key={idx} className="px-5 py-3 text-gray-700 text-[14px] font-medium hover:text-[#800000] hover:bg-red-50 cursor-pointer flex justify-between items-center border-b border-gray-100 transition-colors">
                        {cat}
      ...
      <header className="bg-[#800000] h-14 md:h-20 sticky top-0 z-50 shadow-md transition-all">
        <div className="container mx-auto px-4 h-full flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3 md:hidden w-full">
             <button onClick={() => setIsMobileMenuOpen(true)} className="p-1 -ml-1 active:scale-90 transition-transform">
                <Menu className="text-white w-6 h-6" />
             </button>
             <span className="text-white font-bold text-lg flex-1 truncate">Seasonal Bazar</span>
            ...
            <div className="relative w-28 shrink-0">
              <div className="absolute top-[-30px] left-18 w-24 h-24 lg:w-28 lg:h-28 bg-white rounded-full border-[5px] border-white flex items-center justify-center shadow-lg z-30 overflow-hidden cursor-pointer group">
                 <div className="relative w-full h-full p-3 group-hover:scale-105 transition-transform">
                   <Image src="/SeasonalBazar_Logo.png" alt="Logo" fill className="object-contain" priority />
                 </div>
              </div>
            </div>

            <div className="flex-1 max-w-2xl relative">
              <input type="text" placeholder="Search Seasonal Product..." className="w-full h-10 pl-5 pr-10 rounded-full text-sm focus:outline-none shadow-inner text-black bg-white focus:ring-2 focus:ring-yellow-400 transition-all" />
               <Search className="absolute right-3 top-2.5 text-gray-500 w-5 h-5" />
            </div>

            <div className="flex items-center gap-3">
               <div className="w-10 h-10 bg-white rounded-full flex items-center justify-center cursor-pointer hover:bg-gray-200 transition shadow-sm group"><MapPin className="text-black w-5 h-5 group-hover:text-[#800000]" /></div>
               ...
               <div className="w-10 h-10 bg-white rounded-full flex items-center justify-center cursor-pointer hover:bg-gray-200 transition shadow-sm group"><User className="text-black w-5 h-5 group-hover:text-[#800000]" /></div>
            </div>
          </div>
        </div>
      </header>
      ...
      <div className="container mx-auto px-3 md:px-4 py-4 md:py-6 flex gap-6 flex-1 items-start">
          ...
          <ul className="space-y-0.5">
            {categories.map((cat, idx) => (
              <li key={idx} className="px-5 py-2.5 text-gray-700 text-[14px] font-medium hover:text-[#800000] hover:bg-red-50 cursor-pointer flex justify-between items-center group transition-colors border-l-4 border-transparent hover:border-[#800000]">
                {cat} <ChevronRight className="w-4 h-4 text-gray-300 group-hover:text-[#800000] transition-all group-hover:translate-x-1" />
              </li>
            ))}
          </ul>
        </aside>
        ...
        <main className="flex-1 min-w-0 w-full">
          ...
          {/* Main Hero Banner (Height adjusted to 340px to match sidebar) */}
          <div className="w-full h-[160px] sm:h-[220px] md:h-[340px] rounded-lg md:rounded-xl overflow-hidden relative shadow-md bg-gray-200 group mb-10">
             <div className="absolute inset-0 w-full h-full">
               <Image src="/banner.jpeg" alt="Banner" fill className="object-cover object-center group-hover:scale-105 transition-transform duration-700" priority />
              ...
              <div key={product.id} className="group flex flex-col rounded-lg overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 bg-white border border-gray-100">
                <div className="h-36 md:h-48 w-full bg-white flex items-center justify-center relative p-3 overflow-hidden">
                  <div className="relative w-full h-full">
                    <Image src={product.img} alt={product.title} fill className="object-contain group-hover:scale-110 transition-transform duration-500" />
                  </div>
                ...
                <div className="p-3 flex-1 flex flex-col bg-gray-50/50">
                  <h3 className="font-bold text-gray-800 text-[13px] md:text-[15px] leading-tight mb-1 line-clamp-2 h-9 md:h-10 group-hover:text-[#800000] transition-colors">{product.title}</h3>
                  ...
                  <button className="mt-3 w-full bg-[#800000] text-white text-[10px] md:text-xs font-bold py-1.5 md:py-2 rounded hover:bg-[#600000] transition active:scale-95 md:hidden">ADD TO CART</button>
                </div>
              </div>
            ))}
          </div>
        ...
        {/* Right Sidebar (Aligned with Main Banner) */}
        <aside className="w-64 hidden xl:flex flex-col gap-4 shrink-0 sticky top-24">
          
          {/* Banner 1: Summer Sale (Height 180px) */}
          <div className="bg-[#FFD700] h-[180px] rounded-lg relative overflow-hidden shadow-md group cursor-pointer hover:shadow-xl transition-all w-full flex items-center justify-center text-center">
             <div className="z-10 px-4">
               <h3 className="text-white drop-shadow-md text-4xl font-black italic">Summer<br/>SALE</h3>
          ...
          {/* Banner 2: Flash Sale (Height 144px) */}
          {/* Note: 180px + 144px + 16px(gap) = 340px (Matches Main Banner) */}
          <div className="bg-gradient-to-br from-blue-600 to-blue-500 h-[144px] rounded-lg relative overflow-hidden shadow-md flex items-center justify-between px-4 w-full cursor-pointer hover:brightness-105 transition-all">
             ...
             <div className="z-10 pl-6">
               ...
               <h3 className="font-black text-white italic text-3xl mt-1 uppercase">Sale</h3>
             </div>
             <div className="z-10 text-right">
                <span className="block text-yellow-300 font-black text-2xl drop-shadow-md">-50%</span>
                <button className="bg-white text-blue-700 text-[10px] font-bold px-3 py-1.5 rounded shadow mt-2">BUY NOW</button>
             </div>
          </div>

          {/* Best Deal Grid */}
          <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 w-full mt-2">
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 h-16 flex justify-between items-center px-6 z-50 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)]">
         <div onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })} className="flex flex-col items-center gap-1 text-[#800000] cursor-pointer">
            <HomeIcon className="w-5 h-5" />
            <span className="text-[10px] font-medium">Home</span>
         </div>
         <div onClick={() => setIsMobileMenuOpen(true)} className="flex flex-col items-center gap-1 text-gray-400 hover:text-[#800000] cursor-pointer transition-colors">
      ...
      <footer className="bg-[#6B0F0F] text-white pt-10 border-t-8 border-[#800000] mt-auto hidden md:block">
        <div className="container mx-auto px-4 grid grid-cols-1 md:grid-cols-4 gap-8 text-sm pb-8">
           <div><h4 className="font-bold mb-3 text-base text-yellow-400 border-b border-white/20 inline-block pb-1">Office Address:</h4><p className="leading-relaxed opacity-90 text-xs">Milon Cinema Hall, Puran Bazar,<br/>Madaripur Sadar, Bangladesh</p></div>
           <div><h4 className="font-bold mb-3 text-base text-yellow-400 border-b border-white/20 inline-block pb-1">Contact us:</h4><p className="opacity-90 text-xs">+8801705946829</p><p className="opacity-90 text-xs">info@seasonalbazar.com</p></div>
           <div><h4 className="font-bold mb-3 text-base text-yellow-400 border-b border-white/20 inline-block pb-1">Payment:</h4><p className="opacity-90 text-xs">Bkash, Nagad, Rocket</p></div>
           <div><h4 className="font-bold mb-3 text-base text-yellow-400 border-b border-white/20 inline-block pb-1">Banking:</h4><p className="opacity-90 text-xs">Dutch-Bangla Bank PLC.</p></div>
        </div>
        <div className="bg-[#3E0000] py-4 text-center text-[11px] text-gray-400 border-t border-[#500000]">
          ...
          <p className="mt-1">Development by: <Link href="https://pixelandcode.agency/" target="_blank" className="text-white font-bold cursor-pointer hover:underline">Pixel & Code</Link></p>
        </div>
      </footer>
    </div>
  );
